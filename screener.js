...
function calculate() {
  ...
  const n = loanTerm * 12, r = intRate / 100 / 12, pmtsMade = holdPeriod * 12;
  let loanBalance = intRate > 0
    ? loanAmount * (Math.pow(1+r,n) - Math.pow(1+r,pmtsMade)) / (Math.pow(1+r,n) - 1)
    : loanAmount - (loanAmount / (loanTerm*12)) * pmtsMade;
  ...
  const equityMult   = totalCashOut / totalEquity;

  const setMetric = (id, value, pass) => {
    el(id).textContent = value;
    el(id).closest('.metric-card').className = 'metric-card ' + (pass === null ? '' : pass ? 'good' : 'bad');
  };
  ...
  el('waterfallRows').innerHTML = wfData.map(row => {
    ...
    const barColor = isNeg ? 'var(--accent)' : row.cls === 'wf-noi' ? 'var(--ink)' : 'var(--green)';
    return `<div class="wf-row ${row.cls}">
      ...
      <div class="wf-bar-wrap"><div class="wf-bar"><div class="wf-bar-fill" style="width:${barPct}%;background:${barColor}"></div></div></div>
    </div>`;
  }).join('');
  ...
  const rows = [
    ...
    { label: 'Exit / Sale Proceeds',    vals: years.map((y,i) => i===holdPeriod-1 ? fmt$(saleProceeds) : '—'), cls: 'dcf-cf' },
  ];
